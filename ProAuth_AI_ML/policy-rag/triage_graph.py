...
LangGraph orchestration wiring the evidence pipeline together. This graph
produces a ready-to-consume ML FEATURE ROW, never a triage decision — see
the project memory "agents never decide". The trained ML Triage Model
that actually decides APPROVE / PEND / MORE_INFORMATION lives in the
sibling ml/ subpackage (ml/model.py) and consumes this graph's output;
it is not part of this graph. Do not add a decision field to TriageState.

Five stages now. Policy Evidence Agent and Clinical Agent's fact
EXTRACTION still run in PARALLEL (both only need the original request, no
dependency on each other) — but scoring the extracted facts against a
named clinical checklist needs that checklist's names, and only Policy
Evidence Agent produces those (policyEvidence["clinicalCriteria"]). So a
fan-in stage, clinical_criteria_eval, runs after BOTH parallel branches
complete. document_evidence only needs policyEvidence["requiredDocuments"],
so it runs right after policy_evidence rather than waiting on
clinical_evidence too. Only once BOTH clinical_criteria_eval AND
document_evidence complete does the final deterministic aggregator
(coverage_reasoning — repurposed from an LLM agent into the ML
feature-vector builder, see coverage_reasoning_agent.py's module
docstring) run:

    START ──> policy_evidence ─┬──────────────────────┬──> clinical_criteria_eval ──┬──> coverage_reasoning ──> END
                                └──> document_evidence ─────────────────────────────┘
    START ──> clinical_evidence (extract) ─────────────┘

coverage_reasoning also reads state["policyEvidence"] directly without a
dedicated edge — LangGraph state is cumulative.

Each node is a plain Python function operating on a plain TypedDict state
— no LangChain ChatModel/message-wrapper abstractions are used inside the
nodes; the existing raw-Groq-client agent functions are called basically
unchanged, wrapped only enough to read/write the shared state dict.

document_evidence_node calls document_agent.py's evaluate_document_evidence()
— the real implementation (2026-08-19+), which OCRs whatever files were
actually attached to this request (attachedDocuments) against the
required-documents list. A caller-supplied documentEvidence override
(same manual-override precedent as patientFacts before Clinical Agent
existed) still always wins over running the agent at all — useful for
testing/edge cases.

MongoDB checkpointer — now load-bearing (2026-08-19+), not just wired in.
The Node backend passes the Authorization's own ID as thread_id on every
call (see main.py/triageRoutes.js), so every evaluation of the SAME PA
request — the original submission and every resubmission after it —
shares one checkpoint history. That history is used for real: the three
LLM-calling stages (policy_evidence, clinical_evidence extraction,
clinical_criteria_eval) each check the PREVIOUS checkpoint for this
thread before doing any work, and reuse that stage's prior output outright
when its actual relevant inputs haven't changed — see
_get_previous_state() and each node's own reuse check. This is the
concrete fix for "resubmitting shouldn't burn Groq calls redoing work
nothing changed" (2026-08-19 instruction): a doctor who resubmits only to
attach a missing document skips 2-3 Groq calls entirely, since
requestedService/diagnosis/justification/specialty didn't change — only
document_evidence (cheap, no LLM), coverage_reasoning (free), the ML model
(free), and Companion Agent's final explanation (unavoidable — the
decision itself may have changed) run for real.

This is deliberately NOT LangGraph's interrupt()/resume mechanism (this
graph still runs START-to-END every single call, synchronously) — it's a
manual memoization layer built on top of the checkpointer's stored state,
because "skip a sub-computation whose inputs didn't change" isn't what
interrupt/resume is for. Every reuse decision is conservative by design:
ANY relevant input changing, ANY missing/degraded prior result, or the
prior checkpoint being older than STALE_CHECKPOINT_MAX_AGE forces a fresh
recompute — silently serving stale evidence would violate this project's
core "never fabricate/never serve unverified confidence" discipline far
more than a few extra Groq calls would ever cost.

Implementation detail that matters: the prior run's final state is looked
up via _get_previous_state() ONCE in run_evidence_pipeline(), BEFORE
graph.invoke() starts, and threaded into the initial state as
_previousState — nodes read it from state, not by querying the
checkpointer live during execution. LangGraph checkpoints incrementally
per superstep WITHIN a single invoke() (a checkpoint is written before
any node even runs), so a node calling the checkpointer mid-run would see
THIS run's own in-progress checkpoint as "the latest" — not the prior
completed run — and memoization would silently never fire. Capturing it
once, up front, is what makes "the last COMPLETED run" unambiguous.

Usage:
    python3 triage_graph.py
    # or:
    ...
    result = run_evidence_pipeline(
        ...
        plan_id="PLAN005",
        clinical_justification="Patient has BMI of 42 kg/m^2 and a 5-year history of type 2 diabetes mellitus.",
    )
"""
import os
import uuid
